...
// follow
router.post('/follow/:id', validUser, async (req, res) => {
  try {
    let user = await User.findOne({_id: req.user.id});
    let follow = await User.findOne({_id: req.params.id})
...
// unfollow
router.delete('/follow/:id', validUser, async (req, res) => {
  try {
    let user = await User.findOne({_id: req.user.id});
    let follow = await User.findOne({_id: req.params.id})
    var index = user.follows.indexOf(follow);
    if (index !== -1) {
